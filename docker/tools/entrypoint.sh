...
if command -v bundle 2> /dev/null; then
    BUNDLER_OPTIONS="--gemfile=/var/www/html/Gemfile --path=/var/www/html/data/bundle"
    bundle check $BUNDLER_OPTIONS || bundle install $BUNDLER_OPTIONS;
fi
...
if [ "$1" = "composer" ]; then
    ...
    su www-data -s /bin/bash -c "node_modules/.bin/gulp ${*:2}"
elif [ "$1" = "su" ]; then
    /bin/bash -c "${*:2}"
elif [ "$1" = "mysql" ]; then
    ...
    su www-data -s /bin/bash -c "bundle exec cap ${*:2}"
else
